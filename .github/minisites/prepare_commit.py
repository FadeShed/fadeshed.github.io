...
def main():
    ...
    for p in Path('.github/minisites').glob('*.py'):files[p.as_posix()]=p
    entries=[]
    ...
    commit=api('/git/commits',{'tree':tree['sha'],'parents':[parent],'message':'Publish complete minisites with shared FadeShed navigation\n\nRestore the interactive Pasteberth workspace and configuration generator without a demo version badge. Preserve the complete supplied LightWebPres portal and builder. Harmonize parent/product navigation and refresh Markdown documentation indexes.'})
