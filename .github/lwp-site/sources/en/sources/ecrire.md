...
<!-- lwp:slide -->
slug: serie
kicker: ORGANIZE A COLLECTION
## One article. More than one journey.
summary: A series selects the articles, their order and how they are presented in its index.

The same canonical text can serve a short briefing and a larger reading collection. Your source organization defines that reuse; rebuild the affected series after editing a shared article. This is not an automatic synchronization service.

Keep the source project for authoring. Publish the output directory for readers.
